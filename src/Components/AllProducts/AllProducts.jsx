...
const AllProducts = ({productsData, totalCart, setTotalCart}) => {
  return (
    <div className='grid justify-center gap-4 md:grid-cols-2 xl:grid-cols-3 lg:justify-between'>
      {
        productsData.map(singleProductData => { 
          return  <SingleProductData singleProductData={singleProductData} totalCart={totalCart} setTotalCart={setTotalCart} key={singleProductData.id}/>
        })
      }
    </div>
  )
